...
class Database:
    ...
    async def _initialize_db_schema(self):
        ...
        try:
            ...
            # Create telemetry collection with schema validation
            if "telemetry" not in collection_names:
                logger.info("Creating telemetry collection with schema validation")
                await db.create_collection(
                    ...
                    validationLevel="moderate"  # Allow some flexibility in data
                )
            
            # Create metrics collection for aggregated stats
            if "metrics" not in collection_names:
                logger.info("Creating metrics collection")
                await db.create_collection("metrics")
    ...
    async def _create_indexes(self):
        ...
        try:
            ...
            db.telemetry.create_indexes(telemetry_indexes)
            ...
            db.metrics.create_indexes(metrics_indexes)
    ...
    async def get_metrics(self):
        ...
        if self._use_csv_fallback:
            ...
            try:
                ...
                if os.path.isfile(filename):
                    with open(filename, 'r', newline='') as csvfile:
                        ...
                        rows = list(reader)
                        metrics["requests_total"] = len(rows)
                        
                        # Calculate recent requests
                        one_hour_ago = (datetime.now() - timedelta(hours=1)).isoformat()
                        recent_rows = [r for r in rows if r["timestamp"] >= one_hour_ago]
                        metrics["requests_last_hour"] = len(recent_rows)
                        
                        # Calculate error rate
                        if rows:
                            error_rows = [r for r in rows if int(r["status_code"]) >= 400]
                            metrics["error_rate"] = len(error_rows) / len(rows)
                        ...
                        total_tokens = [int(r["total_tokens"]) for r in rows if r["total_tokens"] and r["total_tokens"].isdigit()]
                        if total_tokens:
            ...
            try:
                ...
                # Average latency
                latency_pipeline = [
                    {"$match": {"metadata.latency_ms": {"$ne": None}}},
                ...
                error_count = await db.telemetry.count_documents({"metadata.status_code": {"$gte": 400}})
                if total_count > 0:
                    metrics["error_rate"] = error_count / total_count
                
                # Token usage
                token_pipeline = [
                    {"$match": {"metadata.total_tokens": {"$ne": None}}},
